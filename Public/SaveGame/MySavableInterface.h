...
public:
	...
	// 【修改原因】：防止时序崩溃。
	// 第一步：仅仅把 JSON 字符串接收并缓存在子系统的内存里，绝对不要在此刻找那个实际存在的物理实体 Actor！
	// 此时世界刚加载一半，目标实体极大概率还未生成，提前寻址必将导致空指针崩溃 (Fatal Error)。
	// 数据解析层（Subsystem/缓存）绝对不允许主动去触碰物理表现层（World/Actor），必须让 Actor 诞生后自己来拿数据！
	virtual void InjectUniversalData(const FString& InJsonString) = 0;
	// 【新增原因】：防止时序崩溃。
	// 第二步：由 GameMode 在场景和角色完全加载完毕后触发，此时子系统可以绝对安全地去寻找 Actor 恢复数值。
	// 实现此接口的子系统，必须在此处（且只能在此处）将刚才缓存的数据，真正应用到游戏世界中。
	// 此时你绝对安全地执行互相依赖的跨系统调用（如：UI 读装备、装备读属性）以及真实的 Actor 物理刷新。
